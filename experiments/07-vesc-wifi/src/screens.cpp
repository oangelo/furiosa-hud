...
static void drawConnStatus(BtState btState, ConnType connType) {
  uint16_t col;
  switch (btState) {
    case BT_CONNECTED:
      col = 0x07E0;
  ...
  const char* label = (connType == CONN_WIFI) ? "WiFi" : "BT";
  lcd.drawString(label, BT_CX - 6, BT_CY);
...
static void drawTopBar(const VescData& data, bool showVoltage) {
  ...
  drawConnStatus(data.btState, data.connType);
}
