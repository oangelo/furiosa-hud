...
static constexpr int TOG_W       = 80;
static constexpr int TOG_X       = 4;

static constexpr int SCAN_W      = 80;
static constexpr int SCAN_X      = 88;
...
static constexpr uint16_t COL_WIFI_CLR = 0x04FF;
...
void bt_screen::drawWifiConnectingStatic() {
  lcd.fillScreen(COL_BG);
  drawHeader();

  lcd.setTextDatum(middle_center);
  lcd.setFont(&fonts::Font4);
  lcd.setTextColor(COL_WIFI_CLR);
  lcd.drawString("Conectando WiFi...", 160, 90);

  lcd.setFont(&fonts::Font2);
  lcd.setTextColor(COL_TEXT);
  lcd.drawString(VESC_WIFI_SSID, 160, 125);

  lcd.setFont(&fonts::Font0);
  lcd.setTextColor(COL_DIM);
  lcd.drawString(VESC_WIFI_HOST, 160, 148);
}
...
static void drawBottomBar(ConnType connType, int count) {
  lcd.fillRoundRect(TOG_X, BTN_Y, TOG_W, BTN_H, 8, COL_TOG_BG);
  lcd.setTextDatum(middle_center);
  lcd.setFont(&fonts::Font2);
  lcd.setTextColor(COL_TOG_TXT);
  lcd.drawString(connTypeLabel(connType), TOG_X + TOG_W / 2, BTN_Y + BTN_H / 2);
...
void bt_screen::drawDeviceListStatic(BtDevice* devices, int count, ConnType connType) {
  lcd.fillScreen(COL_BG);
  drawHeader();
  ...
  for (int v = 0; v < maxVis; v++) {
    ...
    switch (devices[i].type) {
      case CONN_BLE:     typeStr = "BLE"; break;
      case CONN_WIFI:    typeStr = "WiFi"; break;
      default:           typeStr = "BT"; break;
    }
    snprintf(tagBuf, sizeof(tagBuf), "%s %d", typeStr, devices[i].rssi);
    lcd.setTextDatum(middle_right);
    lcd.setFont(&fonts::Font0);
    lcd.setTextColor(devices[i].type == CONN_WIFI ? COL_WIFI_CLR : COL_BT_BLUE);
    lcd.drawString(tagBuf, 310, y + ROW_H / 2);
  }

  drawBottomBar(connType, count);
}
